...
class DataManager:
    ...
    def get_batch(self):
        data = []
        max_retries = 1000  # Safety mechanism to prevent infinite loops
        retries = 0
        ...
        while len(data) < self.batch_size:
            try:
                ...
                try:
                    ...
                    raise RuntimeError("Dataset is empty. Cannot fetch any items.")
            ...
            if len(encoded) > self.block_size:
                ...
                if retries >= max_retries:
                    raise RuntimeError(f"Unable to find valid data after {max_retries} attempts. Dataset might be empty or filtered out.")
