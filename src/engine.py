...
class EvolutionEngine:
    def __init__(self, population_size=2, config=None, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.population_size = population_size
        self.device = device

        # Use default config if none provided, but typically it should be provided
        self.config = config if config is not None else GPTConfig()

        self.population = []
        self.optimizers = []

        for _ in range(population_size):
            model = GPTLanguageModel(self.config)
            model.to(device)
    ...
    def evolve(self, val_losses):
        ...
        for i in range(self.population_size):
            new_model = GPTLanguageModel(self.config)
